...
async def test_nc1_legacy_gate_routes_to_legacy_path():
    """NC-1 (positive control, no mutation): with ``effective_runtime("autonomous")`` LEGACY
    (enable key UNSET, settings.runtime="legacy") the SAME wiring routes every step through
    the legacy ``run_step_via_agent_loop`` and the deep step-runner is NEVER called — proving
    the enable key is what flips the surface deep, and the deep path is byte-neutral off."""
    async with _db_env() as (factory, ws, uid, threads):  # noqa: F841 - threads unused here
        await _seed_tools(factory, ws)
        plan_id = await _seed_read_write_plan(factory, ws, uid)
        ...
        try:
            # Belt-and-suspenders: ensure NO stale enable/override/breaker forces deep.
            await r.delete(
                runtime_breaker.enabled_key(AUTONOMOUS_SURFACE),
                runtime_breaker.override_key(AUTONOMOUS_SURFACE),
                runtime_breaker.breaker_key(AUTONOMOUS_SURFACE),
            )
            async with factory() as db:
                ...
                # Legacy path is exercised via a stub so we assert ROUTING, not agent_loop.
                legacy_spy = AsyncMock(
                    return_value={
                        "status": "completed",
                        "result": "legacy",
            ...
            assert len(deep_calls) == 0  # deep step-runner NEVER called on the legacy gate
            assert legacy_spy.call_count == 2  # both steps routed to the legacy agent loop
            assert result_run.status in ("completed", "partially_completed")
        finally:
            await pool.close()
            await r.aclose()
