...
These tests assert on the CONSTRUCTED CALL (system prompt content + delimited user message),
not on model behavior — the fake client's verdict is scripted and irrelevant to what we prove.
"""
...
async def test_poison_with_fake_closing_tag_cannot_escape_the_fence():
    ...
    client = _fake_client(ok=False, concerns=["x"])
    mw = make_governor_delegate_critique_middleware(
        client=client, redis=None, is_read_only_delegate=True, model="haiku-test"
    )
    await _hook(mw)(_request("task"), _handler_returning(_summary_command(poison)))
