...
boundary over HTTP against a live OpenConnector, and asserts the three
guarantees the adapter exists to provide: a real result comes back, secrets
are stripped, and a second principal cannot use the first's connection.
...
async def test_real_execute_action_and_two_principal_isolation():
    ...
    try:
        ...
        assert "story_ids" in flat or "ok" in flat, f"expected a real HN payload, got {payload!r}"

        # (b) No secret-named keys leaked through the adapter's strip_secrets.
