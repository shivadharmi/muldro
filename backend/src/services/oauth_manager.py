...
class OAuthManager:
    ...
    async def store_token(
        ...
        try:
            ...
            await db.flush()
            await db.commit()
            logger.info("oauth_token_stored", extra={"user_id": user_id, "provider": provider})
        ...
        if token.expires_at and token.expires_at < datetime.now(timezone.utc) + timedelta(
            ...
            if token.refresh_token_encrypted:
                refreshed = await self._refresh_token(
                    provider, f.decrypt(token.refresh_token_encrypted.encode()).decode()
                )
                if refreshed:
            ...
            try:
                async with httpx.AsyncClient() as client:
                    ...
                    if resp.status_code == 200:
                        return resp.json()
                    logger.error(
                        "google_token_refresh_failed: %s %s",
                        resp.status_code,
                        resp.text,
                    )
            except Exception as e:
                logger.error("google_token_refresh_error: %s", e)
            return None

        logger.warning("unsupported_provider_refresh: %s", provider)
        return None
