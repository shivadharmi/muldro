...
- **Step 6A.5 (deep runtime hardening):** On the deep path, Jarvis tools now **actually execute** through a central `jarvis_tool_dispatcher` (`wrap_tool_call`) middleware (`src/deep_runtime/middleware/`) that routes every Jarvis tool call through `ToolExecutor.execute_tool` and normalizes `{"error"|"blocked"}` → `ToolMessage(status="error")` (the frozen `blocked` SSE mapping). Jarvis tools are registered as inert schema **shells** (`deep_runtime/tool_bridge.py`) the model sees but never executes; `capability_scope` stays a **separate outer** security guard (dispatcher is inner). Both exempt deepagents' auto-installed built-ins (`DEEPAGENTS_BUILTIN_NAMES`, drift-guarded against a compiled agent). Soul/role **prompt caching** is preserved by a structured `SystemMessage` (`build_system_message`) — deepagents auto-injects `AnthropicPromptCachingMiddleware`; do not add it manually. A durable **`AsyncPostgresSaver`** (dedicated psycopg3 pool) is built once at app lifespan (gated on `runtime=deep`) and injected via a `checkpointer_provider`; durable-but-inert until 6B. The external **Filesystem MCP was removed** (it collided with deepagents' built-in file tools). Still 6B-owned: the `interrupt()` gate, a persisted stable `thread_id`, the `routes_approvals` resume, and the SSE-on-resume contract.
