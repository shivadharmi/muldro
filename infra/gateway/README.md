...
| `JARVIS_DATABASE_URL` | `connection-adapter` (fallback), Jarvis API process | Point at Jarvis's **existing** Postgres (started by the repo-root `docker-compose.yml`, not by this file). Used by the adapter only when `JARVIS_GATEWAY_DATABASE_URL` is unset, so a local-dev value of `postgresql+asyncpg://jarvis:jarvis@host.docker.internal:5432/jarvis` keeps working with no extra variable. |
| `JARVIS_GATEWAY_DATABASE_URL` | `connection-adapter` | **The same database, addressed as the container sees it** — overrides `JARVIS_DATABASE_URL` for this container only. Required whenever the Jarvis backend's own URL is loopback (`...@127.0.0.1:5432/...`, which is exactly what `infra/user-data.sh` writes on the production host, since the backend runs there as a host process): inside a container `127.0.0.1` is the container itself, so that URL reaches nothing. The failure is silent — the adapter's DB connection is lazy, so the container starts and `docker compose up --wait` reports success while every connection lookup fails at request time. Use `postgresql+asyncpg://jarvis:<password>@host.docker.internal:5432/jarvis`; the compose file adds a `host.docker.internal:host-gateway` `extra_hosts` entry so that name resolves on Linux too. `infra/scripts/deploy.sh` refuses to deploy without it. |
...
- **`JARVIS_TOOLHIVE_VMCP_URL`** needs to be set on the **Jarvis API/worker
  process** itself (not just the adapter) for Jarvis to actually route
...
3. **Point ToolHive at Jarvis's JWKS endpoint** — `<jarvis-api-base>/.well-known/jwks.json`
   ...
   the `<jarvis-api-base>` placeholder in `toolhive-vmcp-gateway.yaml`.
...
5. **Seed the `connection_map` row** for that test principal, per provider —
   ...
   `connection_id` must be the OpenConnector `connectionName` Jarvis minted
   for that connection (`ConnectionService.mint_connection_name`,
   `backend/src/services/connection_service.py`).
6. **Set `JARVIS_TOOLHIVE_VMCP_URL`** on the Jarvis API/worker process (see
   §4) and restart it.
