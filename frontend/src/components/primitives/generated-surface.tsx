...
function SurfaceBody({ surface }: { surface: GeneratedSurface }) {
  ...
  if (kind === "plan" && Array.isArray(data.tasks)) {
  ...
  if (kind === "approval") {
    ...
    return (
      <div className="space-y-2">
        ...
        {d.summary && <p className="text-sm text-t-secondary">{d.summary}</p>}
  ...
  if (kind === "alert") {
    ...
    return (
      <div className={`border-l-4 p-3 rounded-r-[var(--radius-sm)] ${levelColors[level] || levelColors.info}`}>
        ...
        {d.message && <p className="text-sm text-t-secondary mt-1">{d.message}</p>}
      </div>
    );
  }
  ...
  if (kind === "briefing") {
    ...
    return (
      <div className="space-y-2">
        ...
        {d.full_text && <p className="text-sm text-t-secondary">{d.full_text}</p>}
        {d.sections?.map((s, i) => (
          <div key={i}>
