...
If it feels powerful but hard to trust, it is off course.
...
- a calm strategist
- a high-agency assistant with restraint
- a system that earns increasing trust through behavior, not claims
...
### 9. Optimize for long-term trust.
...
- observant without feeling invasive
...
3. surface signal selectively
4. propose before overcommitting
...
This means:
- interfaces appear when needed, not by default
- dynamic UI exists to reduce friction, not to show off generation
- summaries are crisp
- decisions are framed clearly
- action surfaces are simple and intentional
- information is layered so the user can go deeper only when useful
- the system should feel stable even when the work underneath is complex

Clutter is not power.  
Noise is not intelligence.  
Excess motion is not delight.
...
- state actions taken and reasons when relevant
- be brief by default, detailed when needed
...
- a manipulative engagement engine
- a productivity guilt machine
- a dashboard of disconnected agent tricks
- an over-personalized system that feels invasive
- a persona-first experience that sacrifices judgment and usefulness
- a system that optimizes for demos over daily trust
...
- act automatically only in well-defined low-risk zones
- expand scope through explicit user trust and repeated reliability

High-impact actions should demand higher confidence and stronger permission standards.

Autonomy should feel like a carefully widened lane, not a sudden leap.
...
- avoid covering gaps with confident prose
- degrade gracefully
- expose missing context when relevant
- recover with the smallest amount of extra burden on the user
- preserve state and continuity whenever possible
- never turn failure into chaos

A trustworthy system is not one that never fails.  
It is one that fails legibly and recovers responsibly.
...
- choose calmness over activity
- choose trust over speed
- choose restraint over theatrics
- choose usefulness over novelty
- choose visible, understandable behavior over black-box magic

Protect the soul even when optimizing the system.
