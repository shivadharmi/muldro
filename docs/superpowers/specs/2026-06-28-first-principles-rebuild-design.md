...
**Date:** 2026-06-28 · **Status:** Adopted (full, correctness-first); **revised post-final-review** (21 must-fix edits applied). Ready for implementation plan.
...
behavior is thin or dead-wired** — and a 7-agent topology that is a hub-and-spoke ghost. This rebuild
...
| # | Thesis | Adopted as |
|---|---|---|
| **T1** | Collapse the 7-agent zoo | One lead (absorbs today's Planner) + read-only research/perception workers; the other 6 roles → middleware/tools/jobs. **Preserve model/budget specialization** per tool/job. Ship **with** T2 (verification) — collapsing cognition removes inspection seams, so it requires the read-back loop + a delegate-summary critique or it trades coordination cost for undetectable single-agent error. |
| **T2** | Enforced verification | **Risk-gated** read-back (mandatory only when `IRREVERSIBLE`, the shared §4.3 predicate) as an inline step-completion gate. Three terminal states (`TERMINAL_SUCCESS = {completed, completed_unverified}`). |
...
A single Deep Agents lead agent (`create_deep_agent`, direct Anthropic API) is the **target** default
path for the overwhelming majority of turns. *(Today chat runs on `agent_loop`; the chat-path cutover
to this lead is §6 Step 6 — `create_deep_agent` has no live importer yet, so Steps 6–8 land on it only
after that cutover.)* It holds read tools, write-capable tools behind **one gate**, and
...
Write tools are gated by **one** gate raising a LangGraph `interrupt` carrying `ApprovalContext`,
regardless of which loop invoked the tool.

- **Gate topology (Step-0 probe-gated).** Preferred: a `wrap_tool_call` middleware raises
  `interrupt()` from inside the wrapper — but whether that is allowed is an **open substrate question**
  (§8), so a **Step-0 probe** must confirm it; **fallback** = `HumanInTheLoopMiddleware` with a `when=`
  predicate (or a dedicated gate node). The write executes **only on the post-approval resumed pass**,
  never in the same node as the interrupt; because LangGraph **replays the host node on resume**, the
  **Step-1 idempotency ledger is what makes that mandatory replay safe** (a characterization test
  covers the gate-wraps-write case).
- **Irreversibility predicate (pinned, shared).** The gate override and the §4.5 verification trigger
  use the **same** predicate, extracted as one shared helper: `IRREVERSIBLE = (reversible is False) OR
  (blast_radius ∈ {external_single, external_multiple, public})` — set-membership, since the `Literal`
  is `{self, internal, external_single, external_multiple, public}` with no bare `external` member and
  no ordering. `IRREVERSIBLE` forces approval at every trust level.
- **The hard override is net-new code + a behavior delta** (not "promote underused fields").
  `TrustEngine.evaluate()` keys on `risk_level` only (4×4) and has **zero** `reversible`/`blast_radius`
  references today. The override is a **new pre-matrix short-circuit** and a **documented behavior
  change** (a `reversible=false, risk_level=low` capability at `autonomous` trust currently
  auto-executes; after this it gates) — ships with a test. `reversible`/`blast_radius` become a
  **deterministic per-capability registry property** (the RiskAssessor LLM may only refine them
  *upward* — never downgrade a real external write to `self`), so a 24h-cache mislabel can't skip the
  gate or verification.
- **`authorization_source` = provenance taint (net-new; the security-critical fix).** *(Fork resolved:
  provenance taint.)* Every data value carries its origin (`user_message` / `perception` /
  `tool_output` / `retrieved_memory`). A chat write is `direct_user_request` **iff every
  security-relevant arg traces to user-originated content** — entity resolution (`Bob` →
  `bob@acme.com`) **preserves** user-origin; any arg touching perception/tool/retrieved content is
  **tainted → gated**. This makes the prompt-injection threat model *structural* without the unsound
  string-subset matching (entity-resolved/LLM-composed args are never substrings of the message, so
  strict matching would force-approve every legitimate write — the double-prompt failure — and any
  relaxation re-admits banned LLM judgment). **This taint plumbing does NOT exist today** —
  `intent_classifier.classify_intent` returns only `(intent, confidence, sources=service-names)`, no
  per-argument extraction — so it is **net-new, security-critical work** (a §6 step), not "wiring up
  existing code." **Default polarity fails closed:** `authorization_source` is a non-defaulted
  `ApprovalContext` field conceptually defaulting to `autonomous` (gating-required), raised to
  `direct_user_request` only by positive audited derivation; a missing value is a **construction
  error**, never a silent skip. On `direct_user_request` → skip approval; else evaluate trust × risk.
- **Structural rule (free; adopted regardless, and the rescue for the predicate).** Custom / delegated
  / headless / perception writes are `authorization_source = autonomous` **by construction** (their
  args are never the user's literal message) — this bounds the custom-agent write surface and is the
  cheap **phase 1**; the chat-lead taint plumbing is **phase 2**.
...
headless/autonomous, custom write-capable agents) acquires a **cross-process lock keyed at the
resource/capability level** — *not* the over-broad `capability_family` (which would over-serialize
unrelated systems, e.g. Jira + Linear under one family). The current `locking.py` primitive does **not**
meet the requirements as-is, to fix: (a) `pg_advisory_lock` is **session-scoped** and won't span an
approval interrupt or cross processes — so acquire the lock **after approval resolves and only around
the actual external call**, never across the human wait; (b) use a **cross-process backend** (Redis)
with a TTL exceeding the call (the 30s `distributed_lock` default is shorter than approval waits);
(c) **reconcile the three key granularities** — lock(resource/capability) vs trust(capability) vs
idempotency(per-step). A contention test (racing chat + headless write on one capability) is a
...
verified by **read-back before** the step is marked complete — **mandatory only when `IRREVERSIBLE`**
(the shared §4.3 predicate). It replaces the by-fiat terminal-status sites (there are **multiple**:
`step_runner.py` run_step:308, LLM-prose fallback:134, aggregate:249, plus `dag_runner.finalize_step`
:650 which marks `completed` unconditionally — a characterization test forbids any write path emitting
a terminal status without a passing post-condition or an explicit `completed_unverified` verdict).

**Three terminal states — `TERMINAL_SUCCESS = {completed, completed_unverified}`:**
- `completed` — read-back confirmed.
- `partially_completed` — read-back contradicts the expected effect (surfaced). *(Today this exists
  only at run level; it becomes a **step-level** status too.)*
- `completed_unverified` — **NET-NEW**; no confirming read exists / eventually-consistent API / verify
  budget exhausted. Distinct from failure so correct actions are never false-failed, and
  **upgradeable** to `completed` when an async read later confirms (so it is a *non-terminal success*,
  resolving the terminal-vs-upgradeable tension). Replace every literal `status == "completed"` counter
  (`RuntimeProjectionService` progress, `_reconcile_plan_status` rollup) with **`TERMINAL_SUCCESS`
  membership**, or a run whose last step is `completed_unverified` never reaches 100%. Threads through
  transition tables, all `transition_run`/`transition_step` callers, `StepList` icons, projections, and
  `outcome_learner`.

**Trust graduation must count only VERIFIED writes (mis-wired today).** `approved_count` increments at
user-approval time **and** at auto-execution finalize (`trust_gate.record_auto_execution_outcome`,
`dag_runner.py:385`) — which fires **after** `finalize_step` and **before** `run_verification`. So
"exclude `completed_unverified`" is a **partial no-op** against the auto-exec increment; the increment
must be **relocated/gated to fire only on confirmed-verified completion** (Steps 3/6 are sized to move
it, not just thread a status).

Confirming reads are budget-bounded and idempotent on resume; budget exhaustion → `completed_unverified`.
**Post-condition coverage invariant:** any `IRREVERSIBLE` capability **must** have a registered
post-condition — enforced as a **startup validation error** (mirroring `validate_registry()`), so a
new write capability can't silently downgrade to `completed_unverified` on exactly the irreversible
path verification exists for. Deterministic checks; fallback to `completed_unverified` only where no
deterministic read exists.

**Async fast-path loop (specified):** the chat fast-path returns immediately; a **deferred-read
executor** (a `SchedulerLoop` tick) attempts read-back with a **give-up TTL**; on confirmation it
upgrades `completed_unverified → completed`; on divergence after the chat turn ended it raises an
**async-divergence surface** (a proactive-insight escalation reusing the `Notifier` hold-for-briefing
path, since escalate-first assumes a present user).
...
decides whether to run the compensator (itself gated + idempotent). *(Fork resolved: escalate-first.)*
Where no compensator exists, escalate regardless.
...
A lean **always-on core** (identity, active preferences via the existing explicit-injection, compact
world-model summary) is **pinned in the system-prompt / cached-prefix region** so it is never
summarized. **Recent conversation is separate:** it lives in the message window and is protected by
`SummarizationMiddleware`'s `keep=` (configure `keep=`/trigger explicitly) — it is *not* "pinned" the
same way, because the middleware summarizes message history, not the system prompt. Bulky/volatile items
...
- **Execution truth** = slim state row + LangGraph `AsyncPostgresSaver` checkpointer. **This package is
  NOT installed today** (`import langgraph.checkpoint.postgres` → `ModuleNotFoundError`) and was never
  spiked — add `langgraph-checkpoint-postgres` to the dependency set and run a **durability/resume
  spike** (§8). Agent control-flow reads (readiness, resume cursor, dependency checks, idempotency
  lookups) are read-your-writes — **never folded from a lagging projection**.
- **`RuntimeEvent`** (append-only) is the **target** system of record — today `RuntimeProjectionService`
  still reads `TaskRun`/`TaskStep` (soften the present tense). It needs a **monotonic `BIGINT` sequence**
  order column: it has only `occurred_at` (server-default `now()`), which ties within a transaction and
  skews across workers, making `reconcile-from-event-log` impossible. Event persistence must be
  **transactional** with the state change it records (one emitter today swallows the flush failure at
  `debug`). Define the retention contract vs `EvictionService` (which hard-deletes old events).
- **Projections — scoped honestly.** UI / history / analytics are derived projections. Today's
  projection derives from *mutable* `TaskRun`/`TaskStep` and emitted event payloads are thin, so
  "new features = projections not columns" holds for **read/analytics** models (a new connector still
  touches its registries; a new surface still touches renderer+component+mapping); full event-sourced
  rebuild requires **event-payload enrichment + a replay-rebuild test** (Step 5). Slim `TaskRun` but
  **keep the cost rollup denormalized** on the 1:1 row (hot history-list reader); extract `context_pack`
  (by-ref + TTL, with a dereference/expiry render fallback) and `policy_decision`; sweep every hot-path
  reader as Step-5 acceptance.
- **Per-step / per-tool idempotency** = a **per-capability semantic identity key** (or the provider's
  native idempotency token) captured **at first-attempt time** — *not* a hash of full args. The real
  failure is **LLM-recomposed payloads on resume** (a regenerated email body changes a full-arg hash →
  double-fire on exactly the irreversible writes the ledger protects); over-normalization that collapses
  two distinct legitimate writes is the opposite failure. **HARD prerequisite before autonomous
  cutover.** Per-step keys do **not** exist today (only `TaskRun`/`Plan`/`NormalizedEvent`).
- **Durability is per-invocation, not per-node.** LangGraph `durability="sync"` is a `stream()`/
  `invoke()` argument; `create_deep_agent` exposes **no** `durability` param — so run any write-bearing
  invocation with `sync` durability (or route writes through a dedicated write sub-graph). One
  irreversible action per node; never combine an auto-execute side effect with an approval interrupt in
  one node. Single-flight resume lease per run (existing SKIP-LOCKED). Promote the "Checkpoint/DB
  mismatch" warning to reconcile-from-event-log. Treat checkpoint/event payloads as untrusted
  (parameterized SQL, **pin a non-pickle checkpoint serializer**).
...
occurred in this substrate. **Enforcement mechanism (not just a test):** a wrapper that binds
`workspace_id` into the LangGraph `thread_id` **and** the `Store` namespace prefix (the same "write the
enforcement wrapper anyway" conclusion that sank Graphiti), so every checkpointer thread and
agent-queryable world-model tool is workspace-scoped by construction. A **blocking test** asserts a
tool call in workspace A cannot return workspace B data through the Store/checkpointer/projection. Two
concrete in-repo holes to close in Step 0: (1) `find_entity`'s `EntityAlias` subquery is **not**
workspace-filtered (alias-match leak); (2) the `agents` table is **global** (no `workspace_id`) — a real
hole once §5 lets founders create write-capable custom agents. This sits alongside `capability_scope`
wiring as a Phase-0 gate.
...
- Custom-agent writes → **gated-until-trusted** (prior decision), must acquire the write lock, and are
  `authorization_source = autonomous` **by construction** (their args are never the user's literal
  message). `AgentRegistry.create_agent`/`update_agent` must **validate** `capability_scope` (today they
  store arbitrary scope incl. write capabilities, on a global table — see §4.10).
- Chat-write authorization → **provenance taint** (§4.3): the structural rule (autonomous/custom/
  headless = gated by construction) is **phase 1**, the chat-lead taint plumbing is **phase 2**.

---

## 6. Migration order (correctness-first; runtime swap last)

Lowest-risk / highest-correctness-leverage first. Most steps are independently shippable, but two
**coupled bundles** ship together: **{Step 3 ↔ Step 7}** (collapsing cognition needs verification's
inspection seam) and **{Step 7 ↔ Step 8}** (collapsed agents must run on the new context model).
Schema-touching Steps 1/3/4/5 each need a per-step **in-flight-run posture** (drain / dual-read /
reconcile) + a resume-across-deploy test, and Step 5 needs a **data-migration/backfill plan**
(expand-contract, dual-read, pre-cutover event-log-gap handling). A **testing-strategy** deliverable
spans the plan — minimally a write-lock contention test (§4.4) and an end-to-end
verify→partially_completed→escalate→compensate test (§4.5).
...
- **Step 0 — Safety + isolation preconditions (blocking, no behavior change):** wire
  ...
  explicit `middleware=` shape. **Also lands here:** the **interrupt-from-`wrap_tool_call` probe** (with
  the `HumanInTheLoopMiddleware` fallback, §4.3); the **`AsyncPostgresSaver` durability/resume spike +
  non-pickle serializer pin** (package is uninstalled); the two §4.10 tenancy holes (`find_entity`
  alias filter, `agents.workspace_id`); make `capability_scope` **deny on DB-exception** (not just
  `None`) and make `build_deep_agent` **refuse to compile a write-tool agent without the scope
  middleware** (fail-closed at construction + negative test). (Graphiti spike already done → BUILD, §4.6.)
- **Step 1 — Per-step idempotency ledger + semantic identity key** (hard prerequisite before any
  autonomous cutover). Use a **per-capability semantic identity key** (or the provider-native token)
  captured at first attempt — not a full-arg hash — so an **LLM-recomposed payload on resume** can't
  double-fire. `langgraph-checkpoint-postgres` lands here. Acceptance: kill the worker after a write's
  API call but before checkpoint, resume where the recomposed raw args differ, assert the external
...
- **Step 6 — Chat-path Deep Agents cutover + kill Operator + unified gate + write lock + fast-path
  contract:** **first cut the live chat path (`jarvis._call_agent` → `agent_loop`) onto the Deep Agents
  lead** — the low-risk, interactive cutover (a human is present; no unattended replay), **distinct
  from** the dangerous autonomous-durable cutover (Step 10) — so the gate/collapse/context (Steps 6–8)
  land **once** on the target runtime instead of being built on `agent_loop` and re-homed. Then: the
  one gate (probe-gated topology, §4.3) with **provenance-taint `authorization_source`** (structural
  rule phase-1, chat-lead taint plumbing phase-2) + the `IRREVERSIBLE` hard override (net-new
  pre-matrix logic + behavior-delta test); the **cross-process resource/capability write lock**
  acquired post-approval around the external call on all write paths; route mutating fast intents
...
- **Step 9 — A2UI layer split:** keep a **native-stream → `surface_update` translation adapter** during
  the dual-runtime window (the phase machine is emitted from the legacy executor until Step 10, and the
  live frontend consumes exactly these phases) — **defer the phase-machine deletion until after Step
  10**. One interrupt approval event; the `version` field must **drive a real legacy/new render branch**
  (or exploit the 24h `ui_surfaces` TTL and prune types after the window); narrative → markdown; rename
  the layer.
- **Step 10 — Autonomous-path cutover onto LangGraph durable execution** (one-owner-per-fact
  ...
  runtime; start new runs on the new one. (The **chat path was already cut at Step 6**; this is the
  **autonomous** durable cutover only.) **Shadow-compare READ-ONLY decision outputs only — never
  ...
  `JARVIS_RUNTIME` back to legacy on breach — the metrics must exist before this step ships, and
  "verification false-negative rate" needs a measurable proxy (a sampled async re-check, since there
  is no ground truth). Retire the escape hatch after a clean window.
...
| **Destroying "your message = authorization" / leaving the perception-write hole** | **Provenance taint** (user-originated args skip the gate; perception/tool/retrieved-tainted args gated) + structural rule (autonomous/custom/headless = gated by construction) — §4.3 |
...
- **Interrupt probe (resolved in Step 0, not left open):** can a `wrap_tool_call` raise `interrupt()`
  from inside the wrapper, or must the gate be a dedicated node / `HumanInTheLoopMiddleware` with
  `when=`? Load-bearing for §4.3 — Step 0 confirms it with the documented fallback.
- **`AsyncPostgresSaver` durability/resume spike:** the package is uninstalled; prove durable resume +
  exactly-once (with the Step-1 ledger) and **pin a non-pickle checkpoint serializer**. Both runtimes
  share the ledger/event log during the dual-runtime window.
- **`PlanReady` SSE contract:** what the existing Next.js renderer needs from the new event stream
  (frontend rebuild out of scope); ties to the §6 Step-9 native-stream → `surface_update` adapter.
...
- **Re-based by this spec:** agent topology (7 agents → 1 lead + research/perception workers), gating
