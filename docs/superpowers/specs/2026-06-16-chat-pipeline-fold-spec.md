...
| 4 | **Runtime events** | `plan_generated` via `_publish_event`; `run_completed` **awaited** | `plan_created` via `_fire_event` (background) + SSE `plan` | **Batch converges onto canonical `plan_created` fired background via `_fire_event`; drop orphan `plan_generated` — CONFIRMED STALE-LEGACY DRIFT (owner 2026-06-16)**: archaeology shows `plan_generated` (origin `c0883e9`, agent-stream bus) predates the durable runtime-event vocabulary and is consumed by **nothing** (not whitelisted in `routes_realtime`, not metered, not durable); `plan_created` is the canonical event (`runtime_events.py`, `routes_realtime.py` whitelist). Batch *gains* the durable runtime-events record it currently lacks. (Note: `metrics_service.record_plan_created` exists but is **uncalled** from anywhere today — neither path meters plan creation; this convergence does not add a metrics increment, only the durable record + realtime whitelisting) **Firing-discipline scope (clarified 2026-06-16):** once batch folds onto the shared `_process_core`, *all* its runtime events — `command_received`, `route_selected`, `run_completed`, `run_failed` — converge to background firing (previously awaited inline), matching the stream path. This is the intended completion of "one firing discipline (background)", not just the plan event. Trace finalization (`finish_trace`) stays deterministic — the batch adapter drains the core generator rather than early-returning, so the generator's `finally` runs before the result is returned | **Drift** — `plan_generated` vs `plan_created` is almost certainly an accident | Med — telemetry/consumers keyed on event names |
