...
- **Kill-switch storage — RESOLVED (ledger): zero-migration Redis override**, see Phase 4 box. Redis-durable override key + the static `settings.runtime` fail-safe fallback → **zero migration**, preserves "a Redis outage never strands a surface on deep" (outage → falls back to static `legacy`). The DB-table variant is DEAD unless a new requirement appears — force `legacy` even while static settings say `deep` (not the merge-then-flip model), which merge-then-flip never does. See Phase 4.
...
| `src/orchestrator/agent_invoker.py` | (3) additive `run_shadow_turn(...)` + parameterize `_build_deep_agent_for(execute_tool=...)` so the injected `ShadowToolExecutor` is REACHED on BOTH deep (`:293-294`) + legacy (`:611`) dispatch; (4) wire the seam `:528-529` to `effective_runtime("chat")` resolved once. **Cross-touch w/ 10A A4/A6 — single-owner, sequence after 10A** | 3, 4 |
...
- **⚠️ Injection seam (LOAD-BEARING — the write-suppression is only real if the `ShadowToolExecutor` is actually REACHED):** `_build_deep_agent_for` (`agent_invoker.py:201`) has NO `execute_tool` param — it hard-wires `make_jarvis_tool_dispatcher(execute_tool=self._tool_executor.execute_tool)` (`:293-294`), and the legacy path hard-wires `execute_tool_fn=self._tool_executor.execute_tool` (`:611`). As written, a DEEP shadow (the one that runs when authoritative==`legacy` = today's prod default) would dispatch through the REAL executor and silently BYPASS suppression. So `run_shadow_turn` MUST parameterize the executor: add `_build_deep_agent_for(..., execute_tool: ExecuteToolFn | None = None)` defaulting to `self._tool_executor.execute_tool` (ADDITIVE → byte-neutral), and thread the injected `ShadowToolExecutor` through BOTH the deep dispatcher build (`:293-294`) AND the legacy `execute_tool_fn=` (`:611`). **This makes `agent_invoker.py` a cross-touch file in Phase 3 too (not just the Phase-4 seam) — single-owner-per-file, sequence 10B AFTER 10A A4/A6.** The Phase-0 spike wraps a MINIMAL loop, so it does NOT exercise this real build path — a dedicated teeth test (Task 3b) must, exercising the REAL `_build_deep_agent_for`.
...
- [ ] **Step 1: Failing test** — with `shadow_sample_rate=1.0` + fake models for both runtimes, `maybe_run_shadow` runs the shadow, the comparator detects an injected divergence, and `shadow_divergence` is emitted (assert via `generate_metrics()` delta). Second test: inject a shadow that RAISES → `maybe_run_shadow` swallows it (logged), the authoritative turn's captured output is unchanged, and NO exception propagates (isolation contract). Third test: `shadow_sample_rate=0.0` → `run_shadow_turn` is NEVER called (assert on a spy) → byte-neutral. **Fourth test (injection-seam teeth):** a DEEP-shadow write, routed through the REAL `run_shadow_turn` → `_build_deep_agent_for` build path with a SPY tool executor, records ZERO real dispatches — asserting the `ShadowToolExecutor` is actually REACHED inside the deep dispatcher, not bypassed by a hard-wired real executor (this exercises the REAL build path, NOT the Phase-0 prototype's minimal loop).
- [ ] **Step 2: Run → FAIL.**
- [ ] **Step 3: Implement** — `ShadowRunner` (sample + budget-gate + call `run_shadow_turn` + compare + emit); `agent_invoker.run_shadow_turn` (additive build path with explicit runtime + `ShadowToolExecutor` + throwaway `db_factory` session, no idempotency wrap, no surface push); the `chat_processor` `_spawn_background` spawn after `RunCompleted`; default `shadow_sample_rate=0.0` in `settings.py` + `make_mock_settings`. **Parameterize `_build_deep_agent_for(..., execute_tool: ExecuteToolFn | None = None)`** (default `self._tool_executor.execute_tool` → byte-neutral) and thread `run_shadow_turn`'s injected `ShadowToolExecutor` through BOTH the deep dispatcher build (`make_jarvis_tool_dispatcher(execute_tool=...)` at `:293-294`) AND the legacy `execute_tool_fn=` (`:611`), so the shadow's suppression is actually REACHED on either runtime.
- [ ] **Step 4: Run → PASS.**
- [ ] **Step 5: Negative controls (teeth) — two mutations:** (a) remove the try/except isolation in `maybe_run_shadow` → the "shadow raises → authoritative unaffected" test FAILS (proves the isolation is real). (b) revert the `_build_deep_agent_for` parameterization (hard-wire `execute_tool=self._tool_executor.execute_tool`) → the Step-1 DEEP-shadow-write-zero-dispatch test FAILS because the write reaches the REAL executor (proves the injection seam — not the Phase-0 prototype — is load-bearing). Restore both.
...
async def test_redis_error_falls_back_to_static_not_deep():
    # a Redis GET that RAISES must fall through the except-arm to static legacy, NEVER deep (fail-safe)
    assert await effective_runtime("chat", redis=raising_redis(), settings=mk(runtime="legacy")) == "legacy"
...
- [ ] **Step 7: Negative controls (teeth) — three load-bearing guards, one-line mutation each, each caught by a specific test:**
  - (a) **override tier** — make `effective_runtime` ignore the override tier → `test_manual_override_forces_legacy_over_enable` FAILS.
  - (b) **resolve-once-per-turn** — make the gate re-read mid-turn (bypass the resolve-once cache) → `test_resolved_once_per_turn_is_stable` FAILS (proves per-turn stability is real, not incidental — a mid-turn breaker trip must never switch runtime under a running turn).
  - (c) **Redis-error fail-safe** — make the except-arm default to `deep` instead of falling through to static `legacy` → `test_redis_error_falls_back_to_static_not_deep` FAILS. This is the single most safety-critical property: an errant `deep` on a Redis error is the DANGEROUS direction, so the mutation MUST be caught. Restore all three.
...
- **Per-task: a negative control WITH TEETH on every load-bearing guard** — the write-never-dispatched suppression, the DEEP-shadow injection seam (a write reaches the REAL executor if `_build_deep_agent_for` isn't parameterized), the override-forces-legacy priority, the resolve-once-per-turn stability, the Redis-error→`legacy` fail-safe (an errant `deep` on error is the DANGEROUS direction), the shadow-isolation try/except, the signal→surface map. Each is a one-line mutation the guard must defeat (Step-8/9 lesson).
...
5. **10A collision:** Phase 3 parameterizes `_build_deep_agent_for`'s executor (`:293-294`/`:611`) AND Phase 4 edits the `agent_invoker.py:528-529` seam 10A A6 (`:536`) + A4 touch; Phase 2/3 reuse the write-classification 10A A3/A7 hardened. Sequence 10B Phases 3+4 AFTER 10A; single-owner-per-file; RE-VERIFY all anchors (banner).
