...
| `src/orchestrator/jarvis.py` (`generate_briefing`) | Runtime-derive briefing format off `call_agent("presenter")` — retire the Presenter agent's last non-chat caller (B7 prereq) | A-8 |
...
- **⚠️ SPLIT TRAP:** the WS message-type `surface_update` is **overloaded** on the same channel `jarvis:a2ui:{user_id}` — and it is **NOT** a two-publisher problem. At least **FOUR** distinct sites publish to that channel today: `execution_surface_emitter.py:168` (phase-machine `SurfaceUpdate`), `communication_server.py:52` (Presenter's `push_ui_update`), `surface_pusher.py:220`/`:306` (workspace/insight surface pushes), and `notifier.py:447` (notification surface). The adapter must not let a deep phase frame be mistaken for **any** of these non-phase surface pushes (or vice-versa). The Step-1 discriminator (an explicit `kind`/`source` on the envelope, or a separate sub-channel) must disambiguate **ALL** publishers on the channel, not just the phase-vs-Presenter pair — decide in Step 1, do not leave it implicit.
...
> **Confirmed at plan-write:** the **perception_runner** briefing path (`perception_runner.py:387-406`, `:604-624`) does NOT call the presenter *agent* — it stores a briefing memory (`mem_svc.store_briefing_memory`) and surfaces via `SurfaceService`. So perception has no presenter-double-fire concern.
>
> **⚠️ CORRECTION (supersedes an earlier draft that read "chat is the only presenter-agent caller; briefing does NOT use the presenter agent"):** that was FALSE. There is a **SECOND, different** briefing path that DOES call the Presenter *agent*: `jarvis.py:509` `generate_briefing()` calls `self._call_agent("presenter", ...)` (`:553`) to format the daily briefing. It is LIVE + scheduler-driven (`schedule_dispatch.py:70-72` dispatches `action=="generate_briefing"`; `schedule_seeder.py:35` seeds it; also reachable via `intelligence_server/persona.py:82`), **runtime-AGNOSTIC** (no deep branch), and NOT covered by B5 (chat only), R2 (chat flip) or R3 (perception flip). So `generate_briefing` is a live Presenter-*agent* caller that survives **every** surface flip. This means the Presenter *agent-row* drop (B7) is **NOT** gated by chat (or perception) alone: after R5's `DELETE FROM agents WHERE name IN ('presenter',...)` + the commit-1 prune of the `presenter` `AGENT_PROMPTS` key, `self._agents.get("presenter")` → `None` → `ValueError("Unknown agent: presenter")` (`agent_invoker.py:774-776`, swallowed at `jarvis.py:609`) → **scheduled daily briefings silently stop.** Task **A-8** retires this last non-chat caller so the Presenter agent has ZERO callers on the live deep path before R5; if A-8 is deferred, B7 descopes to Librarian-only (6→5, see A-8 and the R5 open-question). (Re-verify at execution that A-8 landed and that NO `call_agent("presenter")` caller remains outside the deep lead.)
...
### Task A-8 — retire the Presenter agent's last non-chat caller (`generate_briefing`)

**Why (verified anchors):** `jarvis.py:509` `generate_briefing()` calls `self._call_agent("presenter", ...)` (`:553`) to format the daily briefing — the **only** live Presenter-*agent* caller outside chat. It is scheduler-driven (`schedule_dispatch.py:70-72` on `action=="generate_briefing"`; seeded `schedule_seeder.py:35`; also reachable via `intelligence_server/persona.py:82`) and **runtime-agnostic** — no B5/B6 deep branch touches it, so R2 (chat flip) and R3 (perception flip) both leave it firing. After R5 drops the `presenter` agent row + prunes its `AGENT_PROMPTS` key, `self._agents.get("presenter")` → `None` → `ValueError("Unknown agent: presenter")` (`agent_invoker.py:774-776`), swallowed at `jarvis.py:609` → **scheduled daily briefings silently stop.** **B7's 6→4 is unsound until this caller is retired.** This mirrors B2/B5's presenter→inline collapse — the deep lead (or a non-agent formatter) does the formatting; there is no separate Presenter agent to route to.

**Files:** `src/orchestrator/jarvis.py` (`generate_briefing` — the `_call_agent("presenter", ...)` at `:553` becomes runtime-derived: on `deep`, format the briefing through the deep lead inline — mirroring B2/B5 — or a non-agent formatter; on `legacy` (default), byte-identical to today; the `_briefing_already_exists` idempotency + delivery block at `:578-605` is unchanged); Test `tests/test_generate_briefing_deep_branch.py`.

- [ ] **Step 1 — failing test:** with `runtime="deep"`, `generate_briefing` does NOT call `_call_agent("presenter")` (assert not called) and still produces a formatted briefing + the single notification + surface push; with `runtime="legacy"` (default), byte-identical to today (presenter agent formats).
- [ ] **Step 2 → 4:** RED → add the single runtime branch (deep lead inline / non-agent formatter) → GREEN.
- [ ] **Step 5 — negative control:** leave the presenter call firing on `deep` → the "no presenter-agent caller on deep" assertion fails. **Step 6:** full gate. **Step 7:** commit `feat(step10d): A-8 retire generate_briefing's presenter-agent caller — runtime-derived briefing format (dormant)`.

> **Owner note:** A-8 edits `jarvis.py` (`generate_briefing`), a **separate owner** from the `agent_invoker.py` A-2..A-6 chain and from `chat_processor.py` (A-5) — it may run in parallel with them. **Prereq for B7 (R5):** with A-8 landed and the deep path live, the Presenter agent has ZERO callers before R5, so the 6→4 row-drop is sound. **⚠️ Fallback if A-8 is deferred/slips:** B7 **DESCOPES to Librarian-only (6→5)** — drop only the `librarian` row at R5 and keep the `presenter` row until `generate_briefing` is migrated (a stranded routed row is harmless; the governor/operator precedent). Record the descope in the ledger; the Presenter row stays as a live route for `generate_briefing`.
...
- [ ] **⚠️ R1 rollback trigger (this is the ONE merge in the entire rebuild — name its failure mode).** At R1 nothing is flipped, so the per-surface gate (already `legacy`) is **not** a lever if the **dormant deploy itself** misbehaves. If the dormant-deploy smoke fails — **any** surface reads ≠ `legacy` on `AGENT_RUNTIME_CALLS`, a byte-neutrality violation slips past R0, or prod error-rate/latency moves off the pre-merge baseline → **redeploy the prior `main` SHA / `git revert` the merge commit, and do NOT proceed to R2.** This git-level rollback is R1-ONLY; once the dormant deploy is confirmed healthy, rollback reverts to the per-surface gate.
- [ ] **Success criteria:** `main` deployed, all surfaces reading `legacy`, prod green, gate + watcher live and reporting. **Once this smoke passes**, the branch is integrated and rollback becomes the per-surface gate (not `git revert`); until it passes, R1's failure mode is the git-level rollback above.
...
**Precondition:** ALL THREE surfaces cleared (chat + perception + autonomous stable on deep) **AND Task A-8 landed** (`generate_briefing` no longer routes to the Presenter agent). Presenter+Librarian are now dead as *agents*: chat uses the deep lead (Presenter collapsed to inline B2 + B5), the daily briefing formats via the deep lead / non-agent formatter (A-8), and perception+chat both extract via the librarian **middleware** (Librarian collapsed). If A-8 did NOT land, this precondition holds for **Librarian only** → apply the R5 open-question fallback (c): drop Librarian (6→5), keep the Presenter row.
...
- [ ] **Step 2 — data-only DELETE migration (commit 2):** copy the shape of `alembic/versions/1a2770a28c39_drop_governor_agent_row.py` (+ `574f6c145bca_drop_operator_agent_row.py`): `Revises = 1a2770a28c39` (the current single head), `upgrade()` = `op.execute("DELETE FROM agents WHERE name IN ('presenter','librarian')")` (idempotent; agents are referenced by **name-string only** — the governor migration's repo-wide sweep confirmed NO `ForeignKey` to `agents.agent_id`/`name`, so no cascade surprise). **Predicate note (do NOT "fix" back):** this DELETE intentionally uses `name`-only, **diverging** from the governor/operator precedent's `agent_id = 'x' OR name = 'x'` — seed rows set `agent_id = f"agt_{ULID()}"` (a ULID, NOT literally the agent name), so only `name` is a stable, matchable identity; the precedent's `agent_id = 'x'` disjunct is harmless dead weight (the precedent migrations say as much). Name-only is idempotent + correct here. `downgrade()` = intentional NO-OP with the same rationale comment. This is **the ONLY Step-10 migration.**
...
- [ ] **Recovery (if R5 strands a live caller — e.g. A-8 was skipped and `generate_briefing` still calls the Presenter agent, or any dropped-agent caller surfaces post-drop):** the DELETE migration's `downgrade()` is an **intentional NO-OP** (see Step 2) — `alembic downgrade` **cannot** restore the rows, by design. Recovery is **code-first**: **revert the commit-1 code-prune** (restores the `presenter`/`librarian` keys in `AGENT_PROMPTS` + the `agents.py` scope/tier dicts) → **restart** → `AgentRegistry.seed_defaults` (`agent_registry.py:59-92`) re-CREATES the missing rows on next boot (seed creates/updates, never deletes). Do NOT hand-write a re-INSERT migration; re-seed is the sanctioned path. (This is also why the recommended sequence is A-8-first: it removes the caller so the drop can never strand it.)
...
> **Open question — split Presenter-drop vs Librarian-drop into two migrations by timing?** The two rows become droppable at **different** milestones. **⚠️ CORRECTION (supersedes an earlier draft that read "Presenter droppable after R2 — chat is the only presenter-agent caller; briefing does NOT use the presenter agent"):** that was FALSE — `jarvis.py:509` `generate_briefing()` calls `_call_agent("presenter")` (`:553`), a live, scheduler-driven, **runtime-agnostic** Presenter-*agent* caller NOT retired by the chat (R2) or perception (R3) flip (see the corrected A-6 note). So **Presenter is droppable only once Task A-8 has retired `generate_briefing`'s presenter call AND chat is cleared (R2)** — NOT at R2 alone. **Librarian** is droppable after **R3** (perception cleared — `perception_runner.py:277` `call_agent("librarian")`). **Options:** (a) **single migration after R3** dropping both (simplest — one code-prune + one migration + one restart; **requires A-8 landed** so the Presenter row is caller-free); (b) **two migrations** — drop Presenter after R2 (**requires A-8**), Librarian after R3 (two code-prunes/migrations/restart-coordinated deploys; matches timing but adds a second head-advancing migration, mild churn); (c) **A-8-deferred fallback** — drop **Librarian only (6→5)** after R3 and **keep the Presenter row** until `generate_briefing` is migrated (a stranded *routed* row is harmless; governor/operator precedent). **Recommendation: (a) single migration after R3, A-8 having landed** — a stranded unrouted row is provably harmless, and one migration honors the "B7 is the ONLY Step-10 migration" constraint. Choose (b) only if an operator wants the Presenter row gone the moment chat clears; choose (c) if A-8 slips. **Decide with the user at execution.**
