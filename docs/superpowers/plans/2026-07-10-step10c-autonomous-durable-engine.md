...
> ## ✅ Phase-0 RESOLUTIONS (recorded at execution 2026-07-11 — HEAD 1dcbc76; spikes committed `83f9b4c`/`95ca426`/`973552d`)
> All four gates resolved to their recommended branches; the plan is **NOT disproven**. Each spike was written+run by a subagent, then re-run + read by the main loop (0.1 read in full for non-vacuousness).
>
> - **SQ1 → Branch A (CONFIRMED).** Per-step `build_deep_agent` under `AsyncPostgresSaver`/`durability="sync"`, killed mid-tool-call, resumed via `ainvoke(None,cfg)` on a fresh compiled graph, fires its write **exactly once** (`spike_effects=1`, `total_dispatch=2` genuine replay, msgpack blobs, ws-bound thread_id). **DAG driver stays.** _Caveat:_ the ledger `identity_key` ordinal is a non-issue for **semantic-keyed** writes (e.g. `email.send`) but a **positional-keyed** cap requires deterministic write ordering across replay — P1 note. A raised dispatcher/gate exception fails the whole `ainvoke` (fail-closed) + leaves a pending task → P1's driver must treat an `ainvoke` raise as "resume this thread", not "abort".
> - **SQ2 → Branch C (CONFIRMED).** Double-gate observed (deep `trust_gate` `interrupt()`s for AUTONOMOUS+write on top of the `dag_runner` step gate). **Branch-C mechanism = a `pre_approved_capabilities: frozenset[str] = frozenset()` param** on `make_trust_gate_middleware` (one line: `if capability in pre_approved_capabilities: return await handler(request)` after the read-only pass) threaded through `_build_deep_agent_for`; the autonomous step seam passes `{step.capability}`. Pre-approved cap short-circuits (no 2nd prompt); an **un-approved** within-step cap still gates (not dead-wired). **NO thread_id change** (58≤64), **NO `GraphInterrupt`→run-pause bridge** (P2 bridge AVOIDED). Chat/resume pass the empty default → byte-neutral.
> - **SQ3 → Branch A (CONFIRMED).** Keep the inline `dag_runner._finalize_with_verification` seam; deep `read_back` stays dormant (`read_fn=None`, `deep_readback_enabled=False`). Deep step output shape = `{"status","result","tools_called","errors"}` + `auth_required`/`error_code`/`provider`/`server` passthrough (the exact keys `_finalize_with_verification`/`_detect_auth_required`/`finalize_step` consume). Read-back unification → **B4/10D**.
> - **SQ4 → Branch A (CONFIRMED, with a BUILD).** Reuse `_build_deep_agent_for(..., authorization_source=AUTONOMOUS)` (the one gated build path) injected into `StepRunner`/`GraphExecutor` via a provider. **BUT the idempotency ledger is NOT in the deep chain** (`grep src/deep_runtime` for `make_idempotent`/`identity_key` = EMPTY, re-confirmed at HEAD): P1 MUST wrap the dispatcher's `execute_tool` with `make_idempotent_execute_tool_fn` (per-step `IdempotencyContext(run_id,step_id,ws)`) via the **10B additive `execute_tool` param** (`agent_invoker.py:218`→`:308`, defaulting to the real executor — now the injection seam) + a positional→keyword adapter (`jarvis_tool_dispatcher.py:66` calls positionally; the ledger fn is keyword-only). Verbatim reuse would double-fire on resume.
> - **Reconcile (0.2 → CONFIRMED).** `rebuild_run_projection` is substrate-blind (folds on event `type` only). **No deep-executor emission gap** — every fold-required event type (`step_started`/`step_completed`/`run_*`) is emitted from the shared `DagRunner`/`GraphExecutor` coordinator that wraps both substrates. _Adjacent (pre-existing, both substrates):_ the `dag_runner.py:174` failed-branch transitions to `failed` WITHOUT emitting `run_failed`, so the fold returns `status=None` — **P4's reconcile consumer must treat `status=None` + `completed<total` as "incomplete → re-pick ready steps", never regress a terminal-success step.**
>
> **P1-P6 hot-file set (single-owner + SYNCHRONOUS):** `trust_gate.py` (Branch-C param), `agent_invoker.py` (autonomous deep-step build method + `pre_approved_capabilities` thread-through), `step_runner.py` (`run_step_via_deep_agent` + gate branch), `graph_executor.py` (provider injection). P2 checkpointer-provider + worker wiring. P3 lease. P4 reconcile. P5 reaper. P6 slim.
...
- **Branch C — hybrid (RECOMMENDED to spike).** Keep the step-level `TrustEngine` gate as the **coarse pre-step pause** (its DB-status pause/resume is the durable, scheduler-driven mechanism the whole autonomous system relies on), AND run the deep chain's `write_lock`/`governor_audit`/`read_back` on the deep runtime, but make the deep `trust_gate` **not double-gate** the already-approved step (finest reconciliation TBD by the spike — e.g. the deep gate short-circuits when a step-level `Approval` already covers the capability for that run/step). Exercises the deep chain without a second prompt and without a bridge rewrite. **Correlation caveat (Branch C):** the deep gate persists `Approval(thread_id, tool_call_id)` while the step gate persists `Approval(run_id, step_id)`; since P1 mints `thread_id = make_thread_id(run.workspace_id)` (which embeds **WORKSPACE ONLY**), a short-circuit that keys off "a step-level `Approval` already covers this run/step" has **no `run_id`/`step_id` in the thread_id to correlate on**. So Branch C may require the autonomous `thread_id` to **also embed `run_id`/`step_id`** — and that extended scheme MUST still route through 10A's `make_thread_id`/`workspace_of_thread_id` so the A6 workspace-assertion keeps holding, and MUST still fit **10A's `String(64)` thread_id length budget**.
- **Gate outcome → phases:** the spike (0.3) runs a `read → write` plan through a per-step deep agent with `authorization_source=autonomous` and observes whether the deep `trust_gate` double-prompts on top of the `dag_runner` step gate. The chosen branch dictates P1's gate wiring and whether P2 must add a `GraphInterrupt`→run-pause bridge (Branch A only). **If Branch C wins,** P1 must additionally resolve the thread_id correlation caveat above — extend the autonomous `thread_id` to embed `run_id`/`step_id` **via 10A's `make_thread_id`/`workspace_of_thread_id`** (A6-preserving) and **within the `String(64)` budget** — so the deep gate's `(thread_id, tool_call_id)` `Approval` can be correlated to the step gate's `(run_id, step_id)` `Approval`. Record the decision + the exact double-gate observation.
...
- [ ] **Step 3 — Implement.**
  ...
  - **Ledger wrapping is NET-NEW BUILD, not verification (the exactly-once linchpin).** The idempotency ledger is **NOT** in the deep chain today (`grep -rn "make_idempotent|IdempotencyLedger|identity_key" src/deep_runtime/` is EMPTY): `_build_deep_agent_for` builds its dispatcher (`agent_invoker.py:283`) with the **RAW un-ledgered** `self._tool_executor.execute_tool`, and the ledger is applied **only** on the legacy autonomous path (`step_runner.py:326` `make_idempotent_execute_tool_fn`) — the chat path passes the raw fn (a deliberate no-op, per that wrapper's own comment). **`write_lock` ≠ ledger:** the lock serializes concurrent writes, but a kill→resume replay RE-ACQUIRES the lock and RE-FIRES; only the ledger's `reserve`/`record_success` on a stable semantic `identity_key` makes LangGraph's AsyncPostgresSaver at-least-once replay exactly-once (per spike 0.1). Therefore the autonomous deep step-executor **MUST** wrap the dispatcher's `execute_tool` with `make_idempotent_execute_tool_fn` (per-step `IdempotencyContext(run_id, step_id, workspace_id)`, semantic `identity_key` stable across resume) — mirroring the legacy `step_runner.py:326` wiring and spike 0.1. **Do NOT reuse `_build_deep_agent_for` verbatim for the autonomous build** (SQ4 Branch A as literally worded): its dispatcher is un-ledgered, so a verbatim reuse would run the autonomous deep agent under AsyncPostgresSaver replay WITHOUT the ledger ⇒ external writes **DOUBLE-FIRE on every mid-step resume** — the exact hole the durable cutover exists to close.
    - **Injection point:** `_build_deep_agent_for` reads `self._tool_executor.execute_tool` **internally (not a param)**, so P1 must EITHER (a) add a per-step `execute_tool` override param to `_build_deep_agent_for` — which touches the shared chat build, making `agent_invoker.py` a **hot cross-touch file** (⇒ **extend the single-owner-per-file note below to include `agent_invoker.py`**) — OR (b) build a **narrow autonomous-only deep builder** that wraps the ledger around the dispatcher's `execute_tool` and leaves the shared chat build untouched.
    - **Signature adapter (reinforces this is a BUILD):** `make_jarvis_tool_dispatcher` calls `execute_tool(name, args, user_id, workspace_id)` **POSITIONALLY** (`jarvis_tool_dispatcher.py:66`), but `make_idempotent_execute_tool_fn`/`make_lock_wrapped_execute_tool_fn` produce fns with **KEYWORD-ONLY** `user_id`/`workspace_id` (`step_runner.py:58` `_wrapped(tool_name, tool_input, *, user_id, workspace_id)`), so the ledger wrapping needs a small **positional→keyword adapter** at the dispatcher seam.
    - Write-lock wrapping still rides the deep `write_lock` middleware — do **NOT** double-lock; the net-new work here is the **ledger** layer, which the deep chain lacks.
- [ ] **Step 4 — Run → PASS.**
...
> **Single-owner-per-file + SYNCHRONOUS dispatch** for `step_runner.py`/`graph_executor.py` (hot files; the 6B F811 / Step-8 P4 lesson). **If P1 takes injection-point option (a)** (a per-step `execute_tool` override param on `_build_deep_agent_for`), `agent_invoker.py` becomes a hot cross-touch file too — it is already mutated by 10A (`:536`) and the 6B/7A middleware — so **add `agent_invoker.py` to the single-owner-per-file set** and sequence its edit synchronously. Option (b) (a narrow autonomous-only deep builder) keeps `agent_invoker.py` out of the hot set.
