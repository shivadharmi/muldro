...
`trust_gate` is **outer**; its `auto_execute_*` verdict is a pass-through (`await handler(request)`), so wherever `permission_gate` is installed the call still reaches it, and it never reads trust. **It is not installed everywhere:** `run_autonomous_deep_step` passes no `permission_mode`, so GraphExecutor DAG steps run with `trust_gate` alone (and with the step's own capability pre-approved, short-circuiting even that).
...
**Consequence, stated plainly — and it splits by entry point.** On a turn that carries a `permission_mode` (chat, and the `process_message` batch entry used by scheduler dispatch and the WS action fallback), a write that is irreversible, external/public, or high-risk is staged at **every** trust level including `autonomous`; graduation-to-silent applies only to what `trust_gate` alone would have gated. On **GraphExecutor DAG steps** no `permission_gate` is installed, so the invariant is carried instead by `pre_approved_capabilities` being **conditional** — `dag_runner` forwards `{step.capability}` only on an auto-execute verdict, and forwards the empty set on `approval_required`, so the deep `trust_gate` evaluates the real tool call and reaches its irreversible-union override. Graduation to `autonomous` does **not** silence an irreversible write there. If a scheduled task appears to have stopped working, check the prepared-work queue before debugging it.
