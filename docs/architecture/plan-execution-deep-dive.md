...
> **On this path, `auto_execute_*` executes.** The DAG gate is a bare `TrustEngine.evaluate` with no irreversible-union override, and `run_autonomous_deep_step` passes **no `permission_mode`** (so `permission_gate` is never installed) — but `pre_approved_capabilities` is **conditional**: `dag_runner` forwards `{step.capability}` **only on an auto-execute verdict** (a gate ran and said yes). On an `approval_required` verdict it forwards the **empty set** and lets the step run anyway, so the deep `trust_gate` evaluates the real tool call and reaches its irreversible-union override — PREPARing the write rather than executing it. Graduation to `autonomous` therefore does **not** silence an irreversible write here. Do not restore an unconditional `pre_approved_capabilities`. Any **other** capability the step's agent reaches for still meets the deep `trust_gate`.
>
