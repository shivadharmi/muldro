...
> **On DAG steps, `auto_execute_*` really does execute.** This gate is a bare `TrustEngine.evaluate` — it has no irreversible-union override. And `run_autonomous_deep_step` builds the step's agent with **no `permission_mode`** (so `permission_gate` is not installed) — but `pre_approved_capabilities` is **conditional**: `dag_runner` forwards `{step.capability}` **only on an auto-execute verdict** (a gate ran and said yes). On an `approval_required` verdict it forwards the **empty set** and lets the step run anyway, so the deep `trust_gate` evaluates the real tool call and reaches its irreversible-union override — PREPARing the write rather than executing it. Graduation to `autonomous` therefore does **not** silence an irreversible write here. Do not restore an unconditional `pre_approved_capabilities`. The deep `trust_gate` still guards any **other** capability the step's agent reaches for.
>
...
**The DAG-level gate carries `presence` too.** `DagRunner` takes `presence` (defaulting to `absent`, because every GraphExecutor run is autonomous) and branches on it: only when it resolves to `interrupt` does `approval_required` call `create_approval_and_pause`. With nobody present it runs the step un-pre-approved so the inner gate stages the real tool call, and the run carries on. A run must never park on a human who is not there — a frozen run's approval expires unanswered and the run is cancelled at 0/N steps. Inside a step, though, the deep `trust_gate` / `permission_gate` middlewares do carry `presence`, and a turn with nobody on it (`presence="absent"`) takes a third option rather than pausing — because pausing would stall the turn or orphan a checkpoint, and executing anyway would be an ungated write.
